...
document.addEventListener('init', function (event) {
    ...
    try {
        if (event.target.id === 'eventPage') {
            ...
            if (idItem===2){
                timeDiv.appendChild(ons._util.createElement('<span><b>'+time+':00 '+event.target.data.idItem*3+'/'+event.target.data.idSection*4+'/2017 </b></br><b>Have a big date with Natasha</b></span>'));
                mainDiv.appendChild(ons._util.createElement('<div style="text-align: left;padding: 5px;width:280px">I must on time !</div>'));
            }//event 2
            else if(idSection===2&&idItem===4){
                timeDiv.appendChild(ons._util.createElement('<span><b>'+time+':00 '+event.target.data.idItem*3+'/'+event.target.data.idSection*4+'/2017 </b></br><b>Go to airport to pick Jason</b></span>'));
                mainDiv.appendChild(ons._util.createElement('<div style="text-align: left;padding: 5px;width: 280px">I can late a little bit, so expensive park in airport.</div>'));
            }//event 3
            else{
                timeDiv.appendChild(ons._util.createElement('<span><b>'+time+':00 '+event.target.data.idItem*3+'/'+event.target.data.idSection*4+'/2017 </b></br><b>Go to final exam</b></span>'));
                mainDiv.appendChild(ons._util.createElement('<div style="text-align: left;padding: 5px;width: 280px">It is a very import day! Do not miss!</div>'));
            }
...
function showEvent(idSection,idItem){
    ...
    if (idItem===2){
        ...
        mainDiv.appendChild(ons._util.createElement('<div class="left" style="text-align: left;padding: 5px;width:280px">I must on time !</div>'));
    }//event 2
    else if(idSection===2&&idItem===4){
        ...
        mainDiv.appendChild(ons._util.createElement('<div style="text-align: left;padding: 5px">I can late a little bit, so expensive park in airport.</div>'));
    }//event 3
    else{
        timeDiv.appendChild(ons._util.createElement('<span><b>'+time+':00 '+idItem*3+'/'+idSection*4+'/2017 </b></br><b>Go to final exam</b></span>'));
        mainDiv.appendChild(ons._util.createElement('<div style="text-align: left;padding: 5px">It is a very import day! Do not miss!</div>'));
    }
